...
The CI blueprint MUST state that local `docker compose` must not be broken, that green CI must not require live LLM/Qdrant/VPS credentials, and that if local pytest is red the inventory/test-job substep MUST stop or document an explicit allowlist rather than silently weakening tests.
