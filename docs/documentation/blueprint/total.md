...
Slice 8X  Ship path           CI → evals → HITL → finish 7P → FE. Index: slice8_X.md · detail: slice8_ci.md, slice8_eval.md, slice8_hitl.md
...
## SLICE 7P — Production Platform (before Slice 8+)

**Goal:** Ship to a real VPS without breaking local Docker. Hosted data plane (Postgres, Redis, Qdrant, R2); thin compute (api + worker + nginx on VPS).

**Full Cursor prompts:** `docs/documentation/blueprint/slice-platform.md`

**Phases:**
```
7P.0–7P.4  Repo shape     prod compose, env contract, soft Qdrant boot, R2 docs
7P.5–7P.6  Deploy ready   runbook, deploy scripts, smoke test
7P.7–7P.8  CI/CD          GitHub Actions test/build → SSH deploy → production gate
```

**Gate (run on VPS after 7P.8):**
```bash
curl https://api.yourdomain.com/health                    # 200, db+redis ok
SMOKE_BASE_URL=https://api.yourdomain.com python scripts/smoke_prod.py   # PASS
```

**Infra law for this slice:**
- `docker-compose.yml` — local dev full stack (unchanged behavior)
...
- Slice 7P production gate passed (or local-only experimentation)
