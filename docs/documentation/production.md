...
|-------------|---------------------|
| api, worker, migrate, Caddy (TLS), optional slim prometheus | Supabase, Upstash, Redis Cloud, Qdrant Cloud, R2, Grafana Cloud |
...
| 7P.1 | Env contract | ⬜ next | `.env.example`, `.env.production.example`, `system.md` tier table |
...
| `docker-compose.prod.yml` | 7P.2 — VPS profile |
...
| `nginx/default.conf` | Dev proxy; prod may use host Caddy instead |
...
### 7P.1 ⬜ pending
- Sync `.env.example` ↔ `config.py`.
- Create `.env.production.example` from topology table above.
- Append dependency tiers to `system.md`.
